...
const DonutChart: React.FC<DonutChartProps> = ({ stats, size = 140, strokeWidth = 18 }) => {
    const { new: newCount, acknowledged, resolved, total } = stats;
    ...
    const resolvedOffset = newPercent + acknowledgedPercent;
    ...
    const segments = [
        ...
        { percent: resolvedPercent, color: 'text-green-400', offset: resolvedOffset, label: 'Resolved', value: resolved },
    ];
    
    return (
        <div className="h-full flex flex-col">
             <h3 className="text-2xl font-bold text-primary dark:text-dark-primary mb-4 text-center flex-shrink-0">Alerts Overview</h3>
             <div className="flex-grow flex flex-col sm:flex-row items-center justify-center gap-6">
                <div className="relative" style={{width: size, height: size}}>
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
                        ...
                        <span className="text-sm text-secondary dark:text-dark-secondary -mt-1">Total Alerts</span>
                    </div>
                </div>
