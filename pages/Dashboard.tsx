...
import { Notification, NotificationPriority, Topic } from '../src/types';
                ...
                <div className="flex items-center space-x-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-secondary dark:text-dark-secondary">{notification.message}</p>
                         {isSnoozed && <p className="text-xs text-gray-500 dark:text-gray-500 italic mt-1">Snoozed until {new Date(notification.snoozedUntil!).toLocaleTimeString()}</p>}
                    </div>
                </div>
            ...
            {expanded && (
                <div className="mt-4 pt-4 border-t border-border dark:border-dark-border">
                    <div className="space-y-3 mb-4 max-h-40 overflow-y-auto pr-2">
                        {notification.comments.length > 0 ? notification.comments.map((c, i) => (
...
const Dashboard: React.FC = () => {
    ...
    const stats = useMemo(() => {
        const now = new Date();
        const activeNotifications = notifications.filter(n => !n.snoozedUntil || new Date(n.snoozedUntil) < now);
        return {
            total: activeNotifications.length,
            new: activeNotifications.filter(n => !n.acknowledged && !n.resolved).length,
            acknowledged: activeNotifications.filter(n => n.acknowledged && !n.resolved).length,
            resolved: activeNotifications.filter(n => n.resolved).length,
        };
    }, [notifications]);
    ...
    const filteredNotifications = useMemo(() => {
        ...
        if (statusFilter === 'new') {
            filtered = filtered.filter(n => !n.acknowledged && !n.resolved && (!n.snoozedUntil || new Date(n.snoozedUntil) < new Date()));
        } else if (statusFilter === 'snoozed') {
            filtered = filtered.filter(n => n.snoozedUntil && new Date(n.snoozedUntil) > new Date());
        ...
        const now = Date.now();
        if (timeFilter === '1h') {
            filtered = filtered.filter(n => now - new Date(n.timestamp).getTime() < 3600 * 1000);
        } else if (timeFilter === '24h') {
            filtered = filtered.filter(n => now - new Date(n.timestamp).getTime() < 24 * 3600 * 1000);
        } else if (timeFilter === '7d') {
            filtered = filtered.filter(n => now - new Date(n.timestamp).getTime() < 7 * 24 * 3600 * 1000);
    ...
    const handleCreateTopic = (name: string, description: string) => {
        const newTopic: Topic = {
            id: `t${Date.now()}`, name, description, subscribed: true,
            endpoint: `https://mcm-alerts.app/api/notifications/${name.toLowerCase().replace(/\s+/g, '-')}`
        };
                                ...
                                <h2 className="text-3xl font-bold text-primary dark:text-dark-primary">Recent Notifications</h2>
                                 <div className="flex items-center space-x-2 flex-wrap gap-2">
                                    ...
                                    <FilterSelect onChange={(e) => setStatusFilter(e.target.value)} value={statusFilter}>
                                        <option value="all">Active</option> <option value="new">New</option> <option value="snoozed">Snoozed</option> <option value="resolved">Resolved</option>
                                    </FilterSelect>
                                </div>
                            </div>
                             {timeFilter === 'custom' && (
                ...
                <div key="subscriptions">
                     <GridItemWrapper isEditMode={isEditMode}>
                        <div className="h-full flex flex-col">
                                   ...
                                   <div key={topic.id} className="pb-2 border-b last:border-b-0 border-border dark:border-dark-border">
                                       <div className="flex items-center justify-between">
                                           ...
                                           <label className="relative inline-flex items-center cursor-pointer">
                                                <input type="checkbox" checked={topic.subscribed} onChange={() => setTopics(topics.map(t => t.id === topic.id ? {...t, subscribed: !t.subscribed} : t))} className="sr-only peer" />
                            ...
                            <CreateTopicForm onAddTopic={handleCreateTopic} />
                        </div>
                    </GridItemWrapper>
                </div>
            </GridLayout>
